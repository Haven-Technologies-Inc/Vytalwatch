...
export interface RiskScore {
  ...
  predictions: { event: string; probability: number; preventable: boolean }[];
...
export class RiskScoringService {
  calculateRiskScore(vitals: VitalReading[], alerts: Alert[], age?: number): RiskScore {
    ...
    factors.push({
      name: 'Vital Compliance',
      score: Math.max(0, 100 - (30 - recentVitals.length) * 3),
      weight: 0.2,
    });
    factors.push({ name: 'Trend Analysis', score: this.analyzeTrends(recentVitals), weight: 0.3 });
    if (age)
      factors.push({ name: 'Age Factor', score: age > 65 ? 40 : age > 50 ? 20 : 10, weight: 0.2 });
    ...
    return {
      ...
      riskLevel: level,
      factors,
      predictions: this.getPredictions(level),
      recommendations: this.getRecommendations(level),
      confidence: Math.min(95, 60 + recentVitals.length),
    };
  }
  ...
  private getPredictions(
    level: string,
  ): { event: string; probability: number; preventable: boolean }[] {
    const base = [
      {
        event: 'Hospital Readmission',
        probability: level === 'CRITICAL' ? 0.45 : level === 'HIGH' ? 0.25 : 0.1,
        preventable: true,
      },
    ];
    if (level === 'CRITICAL' || level === 'HIGH')
      base.push({
        event: 'ER Visit',
        probability: level === 'CRITICAL' ? 0.6 : 0.3,
        preventable: true,
      });
    return base;
  }
