...
import {
  ...
  SOAPNote,
  TriageSummary,
} from './charting-assistant.service';
...
export class AdvancedAIController {
  constructor(
    private readonly charting: ChartingAssistantService,
  ...
  async generateOutreach(
    @Body() body: { context: ChartingContext; reason: string },
  ): Promise<{ data: any }> {
  ...
  @Roles(UserRole.PROVIDER, UserRole.ADMIN)
  async calculateRisk(
    @Body() body: { vitals: any[]; alerts: any[]; age?: number },
  ): Promise<{ data: RiskScore }> {
