...
export class EmailController {
  ...
  async sendTestEmail(@Body() dto: TestEmailDto) {
    const templates: Record<string, () => Promise<unknown>> = {
      ...
      magicLink: () =>
        this.emailService.sendMagicLink(
          { email: dto.to, firstName: dto.data?.firstName || 'User' },
          dto.data?.token || 'magic-token-123',
        ),
      healthAlert: () =>
        this.emailService.sendHealthAlert(
          { email: dto.to, firstName: dto.data?.firstName || 'User' },
          {
            id: 'alert-123',
      ...
      inviteCode: () =>
        this.emailService.sendInviteCode(dto.to, {
      ...
      deviceShipped: () =>
        this.emailService.sendDeviceShipped(
          { email: dto.to, firstName: dto.data?.firstName || 'User' },
          {
    ...
    return {
      templates: [
        ...
        { id: 'healthAlert', name: 'Health Alert', description: 'Critical/warning health alerts' },
        {
          ...
          description: 'Upcoming appointment notification',
        },
        {
        ...
        },
        {
          id: 'monthlyReport',
          name: 'Monthly Report',
          description: 'Monthly health summary report',
        },
        { id: 'inviteCode', name: 'Invite Code', description: 'Provider/staff invitation' },
        {
          id: 'deviceShipped',
          name: 'Device Shipped',
          description: 'Device shipping notification',
        },
      ],
    };
  }
