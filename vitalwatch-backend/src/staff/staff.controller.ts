...
export class StaffController {
  ...
  async createRole(@Request() req: any, @Body() body: { name: string; description?: string; permissions: string[] }) {
    return this.staffService.createRole({ ...body, createdById: req.user.sub });
  }

  @Get('roles')
  @Roles(UserRole.SUPERADMIN, UserRole.ADMIN)
  async findAllRoles(@Query('status') status?: StaffRoleStatus, @Query('page') page?: number, @Query('limit') limit?: number) {
  ...
  @UseGuards(PermissionsGuard)
  @RequirePermissions(Permission.STAFF_ROLES_MANAGE)
  async updateRole(@Param('id', ParseUUIDPipe) id: string, @Request() req: any, @Body() body: { name?: string; description?: string; permissions?: string[]; status?: StaffRoleStatus }) {
  ...
  async createStaffMember(@Request() req: any, @Body() body: { userId: string; staffRoleId: string; department?: string; title?: string; employeeId?: string; supervisorId?: string; hireDate?: Date }) {
    return this.staffService.createStaffMember({ ...body, createdById: req.user.sub });
  }

  @Get('members')
  @Roles(UserRole.SUPERADMIN, UserRole.ADMIN)
  async findAllStaffMembers(@Query('status') status?: StaffStatus, @Query('roleId') roleId?: string, @Query('department') department?: string, @Query('page') page?: number, @Query('limit') limit?: number) {
  ...
  async updateStaffMember(@Param('id', ParseUUIDPipe) id: string, @Request() req: any, @Body() body: { staffRoleId?: string; status?: StaffStatus; department?: string; title?: string; additionalPermissions?: string[]; restrictedPermissions?: string[]; supervisorId?: string }) {
  ...
  async getAllPermissions() {
    return { permissions: this.staffService.getAllPermissions(), groups: this.staffService.getPermissionGroups(), categories: this.staffService.getPermissionCategories() };
  }

  @Get('my-permissions')
  async getMyPermissions(@Request() req: any) {
    if (req.user.role === UserRole.SUPERADMIN) return { permissions: Object.values(this.staffService.getAllPermissions()), isSuperAdmin: true };
