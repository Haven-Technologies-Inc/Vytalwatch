...
export class StaffService {
  ...
  async createRole(data: {
    ...
    await this.auditService.log({
      ...
      details: { roleId: saved.id, name: saved.name },
    });
    return saved;
  }
  ...
  async updateRole(
    id: string,
    data: { name?: string; description?: string; permissions?: string[]; status?: StaffRoleStatus },
    userId: string,
  ): Promise<StaffRole> {
  ...
  async deleteRole(id: string, userId: string): Promise<void> {
    ...
    if (membersCount > 0)
      throw new BadRequestException(`Cannot delete role with ${membersCount} assigned members`);
  ...
  async findAllRoles(params?: {
    ...
    page?: number;
    limit?: number;
  }): Promise<{ data: StaffRole[]; total: number }> {
    const page = params?.page || 1;
    const limit = params?.limit || 50;
    const where: any = {};
    if (params?.status) where.status = params.status;

    const [data, total] = await this.staffRoleRepository.findAndCount({
      where,
      skip: (page - 1) * limit,
      take: limit,
      order: { name: 'ASC' },
  ...
  async createStaffMember(data: {
    ...
    await this.auditService.log({
      ...
      userId: data.createdById,
      details: { staffId: saved.id, userId: data.userId, roleId: data.staffRoleId },
    });
  ...
  async updateStaffMember(
    ...
    await this.auditService.log({
      ...
      userId,
      details: { staffId: id, changes: data },
    });
  ...
  async deleteStaffMember(id: string, userId: string): Promise<void> {
    ...
    await this.auditService.log({
      ...
      userId,
      details: { staffId: id, memberUserId: member.userId },
    });
  }

  async findAllStaffMembers(params?: {
    status?: StaffStatus;
    roleId?: string;
    department?: string;
    page?: number;
  ...
  getAllPermissions(): typeof Permission {
    return Permission;
  }
  getPermissionGroups(): typeof PERMISSION_GROUPS {
  ...
  async seedSystemRoles(): Promise<void> {
    const systemRoles = [
      ...
      {
        name: 'Administrator',
        description: 'Administrative access without critical settings',
        permissions: ALL_PERMISSIONS.filter(
          (p) => !p.includes('staff:roles') && !p.includes('settings:manage'),
        ),
        isSystem: true,
      },
      {
        ...
        permissions: [
          ...PERMISSION_GROUPS[PermissionCategory.BILLING],
          ...PERMISSION_GROUPS[PermissionCategory.CLAIMS],
      ...
      {
        name: 'Clinical Staff',
        description: 'Patient and vitals access',
        permissions: [
      ...
      {
        name: 'Support Staff',
        description: 'Basic read access for support',
        permissions: [
      ...
      },
      {
        ...
        permissions: [
          ...PERMISSION_GROUPS[PermissionCategory.COMPLIANCE],
          ...PERMISSION_GROUPS[PermissionCategory.AUDIT],
          Permission.REPORTS_READ,
          Permission.REPORTS_EXPORT,
        ],
        isSystem: true,
