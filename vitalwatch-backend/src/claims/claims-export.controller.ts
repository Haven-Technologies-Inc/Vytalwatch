...
export class ClaimsExportController {
  constructor(
    ...
    private readonly audit: AuditBundleService,
  ) {}
  ...
  async export837P(
    ...
    body: {
      claims: any[];
      submitter: { name: string; ein: string; npi: string };
  ...
  async preview837P(
    ...
    return {
      data: {
        ...
        claimCount: file.claimIds.length,
        totalCharges: file.totalCharges,
      },
    };
  }
  ...
  async downloadAuditBundle(@Body() data: AuditBundleData, @Res() res: Response) {
    ...
    res.setHeader(
      'Content-Disposition',
      'attachment; filename=audit_bundle_' + bundle.claimId + '.pdf',
    );
