...
export interface AuditBundleData {
  ...
  notes: { date: string; content: string; signedBy: string }[];
  alerts: { date: string; type: string; severity: string }[];
  communications: { date: string; type: string; summary: string }[];
}
...
export class AuditBundleService {
  ...
  private generateHash(data: AuditBundleData): string {
    const content = JSON.stringify({
      ...
      timeMinutes: data.timeEntries.reduce((s, t) => s + (t.minutes || 0), 0),
  ...
  private async generatePDF(data: AuditBundleData, hash: string): Promise<Buffer> {
    return new Promise((resolve) => {
      ...
      if (data.claim.codes?.length)
        doc.text('Codes: ' + data.claim.codes.map((c) => c.code).join(', '));
      doc.moveDown();
      ...
      doc
        .fontSize(14)
        .font('Helvetica-Bold')
        .text('TIME DOCUMENTATION (' + totalMin + ' minutes)');
      doc.moveTo(50, doc.y).lineTo(550, doc.y).stroke();
      doc.moveDown(0.5);
      doc.fontSize(10).font('Helvetica');
      data.timeEntries
        .slice(0, 15)
        .forEach((t) => doc.text(' ' + t.category + ': ' + t.minutes + ' min'));
      doc.moveDown();

      // Vitals Summary
      const uniqueDays = new Set(data.vitals.map((v) => new Date(v.recordedAt).toDateString()))
        .size;
      doc
        .fontSize(14)
        .font('Helvetica-Bold')
        .text('VITAL READINGS (' + data.vitals.length + ' readings, ' + uniqueDays + ' days)');
      doc.moveTo(50, doc.y).lineTo(550, doc.y).stroke();
      doc.moveDown(0.5);
      doc.fontSize(10).font('Helvetica');
      const byType = data.vitals.reduce(
        (a, v) => {
      ...
      doc
        .fontSize(14)
        .font('Helvetica-Bold')
        .text('CLINICAL NOTES (' + data.notes.length + ')');
      doc.moveTo(50, doc.y).lineTo(550, doc.y).stroke();
      doc.moveDown(0.5);
      doc.fontSize(10).font('Helvetica');
      ...
      doc.fontSize(10).font('Helvetica');
      doc.text(
        'I hereby certify that all services documented in this audit bundle were rendered as described and are accurate to the best of my knowledge.',
      );
      doc.moveDown(2);
