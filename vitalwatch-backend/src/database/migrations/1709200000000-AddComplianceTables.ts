...
export class AddComplianceTables1709200000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      CREATE TYPE "phi_access_type_enum" AS ENUM (
    ...
    await queryRunner.query(`
      CREATE TYPE "phi_resource_type_enum" AS ENUM (
    ...
    await queryRunner.query(`
      CREATE INDEX "IDX_phi_logs_user_created" ON "phi_access_logs" ("userId", "createdAt")
    `);
    await queryRunner.query(`
      CREATE INDEX "IDX_phi_logs_patient_created" ON "phi_access_logs" ("patientId", "createdAt")
    `);
    ...
    await queryRunner.query(`
      CREATE INDEX "IDX_consents_patient_type" ON "patient_consents" ("patientId", "consentType")
    `);

    // BAA Status enum
    await queryRunner.query(`
      CREATE TYPE "baa_status_enum" AS ENUM (
    ...
    // BAA Party Type enum
    await queryRunner.query(`
      CREATE TYPE "baa_party_type_enum" AS ENUM (
    ...
    await queryRunner.query(`
      CREATE TYPE "retention_action_enum" AS ENUM ('archive', 'delete', 'anonymize')
    `);
    ...
    await queryRunner.query(`
      ...
      VALUES
        ('Vital Readings', 'vital_reading', 2555, 'archive', 'HIPAA requires 7 years retention'),
        ('Clinical Notes', 'clinical_note', 2555, 'archive', 'HIPAA requires 7 years retention'),
        ('PHI Access Logs', 'phi_access_log', 2190, 'archive', 'HIPAA requires 6 years for audit logs'),
        ('Claims', 'claim', 2555, 'archive', 'HIPAA requires 7 years retention')
    `);
  }
