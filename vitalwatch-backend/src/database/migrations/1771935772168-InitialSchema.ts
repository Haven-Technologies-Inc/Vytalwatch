...
export class InitialSchema1771935772168 implements MigrationInterface {
    ...
    private async safeCreateType(qr: QueryRunner, sql: string): Promise<void> {
        await qr.query(`DO $$ BEGIN ${sql}; EXCEPTION WHEN duplicate_object THEN NULL; END $$`);
    }

    private async safeAddColumn(qr: QueryRunner, table: string, col: string, def: string): Promise<void> {
        const r = await qr.query(`SELECT column_name FROM information_schema.columns WHERE table_name='${table}' AND column_name='${col}'`);
        if (r.length === 0) await qr.query(`ALTER TABLE "${table}" ADD "${col}" ${def}`);
    }

    public async up(queryRunner: QueryRunner): Promise<void> {
        await this.safeCreateType(queryRunner, `CREATE TYPE "public"."time_entries_category_enum" AS ENUM('monitoring', 'outreach', 'documentation', 'care_coordination', 'education')`);
        await this.safeCreateType(queryRunner, `CREATE TYPE "public"."time_entries_source_enum" AS ENUM('manual', 'assisted', 'system')`);
        await this.safeCreateType(queryRunner, `CREATE TYPE "public"."time_entries_status_enum" AS ENUM('draft', 'confirmed', 'billed')`);
        await queryRunner.query(`CREATE TABLE IF NOT EXISTS "time_entries" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "patientId" uuid NOT NULL, "userId" uuid NOT NULL, "enrollmentId" uuid, "programType" character varying, "startAt" TIMESTAMP NOT NULL, "endAt" TIMESTAMP, "minutes" integer NOT NULL DEFAULT '0', "category" "public"."time_entries_category_enum" NOT NULL, "source" "public"."time_entries_source_enum" NOT NULL DEFAULT 'manual', "taskId" uuid, "communicationLogId" uuid, "noteId" uuid, "evidenceRefs" text, "billable" boolean NOT NULL DEFAULT true, "cptCode" character varying, "status" "public"."time_entries_status_enum" NOT NULL DEFAULT 'draft', "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_b8bc5f10269ba2fe88708904aa0" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE INDEX IF NOT EXISTS "IDX_7b179edfc496340bb52aa6f4fa" ON "time_entries" ("userId", "createdAt") `);
        await queryRunner.query(`CREATE INDEX IF NOT EXISTS "IDX_fe5c4ded8ae61869d816a15465" ON "time_entries" ("patientId", "createdAt") `);
        await this.safeCreateType(queryRunner, `CREATE TYPE "public"."tasks_type_enum" AS ENUM('review', 'outreach', 'escalation', 'setup', 'billing_review')`);
        await this.safeCreateType(queryRunner, `CREATE TYPE "public"."tasks_priority_enum" AS ENUM('low', 'medium', 'high', 'urgent')`);
        await this.safeCreateType(queryRunner, `CREATE TYPE "public"."tasks_status_enum" AS ENUM('pending', 'in_progress', 'completed', 'cancelled')`);
        ...
        await queryRunner.query(`CREATE INDEX IF NOT EXISTS "IDX_1aa2ea5bb323043bab51bbdfbc" ON "tasks" ("assignedToUserId", "status") `);
        await queryRunner.query(`CREATE INDEX IF NOT EXISTS "IDX_bdb2dc49204bc5229c8cab5e17" ON "tasks" ("patientId", "status") `);
        await queryRunner.query(`CREATE TABLE IF NOT EXISTS "threshold_policies" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "clinicId" uuid NOT NULL, "programType" character varying NOT NULL, "version" integer NOT NULL DEFAULT '1', "isActive" boolean NOT NULL DEFAULT true, "rules" jsonb NOT NULL, "effectiveFrom" TIMESTAMP NOT NULL, "effectiveTo" TIMESTAMP, "createdBy" uuid NOT NULL, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_3680e23497ba63477f05b2dbe6b" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE INDEX IF NOT EXISTS "IDX_c6084d968cb7fc1b4c3e7aaf86" ON "threshold_policies" ("isActive") `);
        await queryRunner.query(`CREATE INDEX IF NOT EXISTS "IDX_8541053809c82c7fff17751f38" ON "threshold_policies" ("clinicId", "programType") `);
        await this.safeCreateType(queryRunner, `CREATE TYPE "public"."enrollments_programtype_enum" AS ENUM('BP', 'GLUCOSE')`);
        await this.safeCreateType(queryRunner, `CREATE TYPE "public"."enrollments_status_enum" AS ENUM('active', 'paused', 'completed', 'cancelled')`);
        await queryRunner.query(`CREATE TABLE IF NOT EXISTS "enrollments" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "patientId" uuid NOT NULL, "clinicId" uuid NOT NULL, "programType" "public"."enrollments_programtype_enum" NOT NULL, "startDate" date NOT NULL, "endDate" date, "status" "public"."enrollments_status_enum" NOT NULL DEFAULT 'active', "orderingProviderId" uuid NOT NULL, "supervisingProviderId" uuid, "currentBillingPeriodStart" date NOT NULL, "currentBillingPeriodEnd" date NOT NULL, "setupCompleted" boolean NOT NULL DEFAULT false, "setupCompletedAt" TIMESTAMP, "setupNoteId" uuid, "deviceId" uuid, "diagnosisCodes" text, "metadata" jsonb, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_7c0f752f9fb68bf6ed7367ab00f" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE INDEX IF NOT EXISTS "IDX_850ae0fc626d9b1c6aa28cae24" ON "enrollments" ("clinicId", "status") `);
        await queryRunner.query(`CREATE INDEX IF NOT EXISTS "IDX_172a52997ee7059344faa6c1f7" ON "enrollments" ("patientId", "programType") `);
        await this.safeCreateType(queryRunner, `CREATE TYPE "public"."claims_status_enum" AS ENUM('draft', 'ready', 'submitted', 'paid', 'denied', 'void')`);
        await queryRunner.query(`CREATE TABLE IF NOT EXISTS "claims" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "patientId" uuid NOT NULL, "enrollmentId" uuid NOT NULL, "periodStart" date NOT NULL, "periodEnd" date NOT NULL, "programType" character varying NOT NULL, "codes" jsonb NOT NULL, "status" "public"."claims_status_enum" NOT NULL DEFAULT 'draft', "readinessChecks" jsonb NOT NULL, "supportingBundleId" uuid, "supportingBundleHash" character varying, "submittedAt" TIMESTAMP, "paidAt" TIMESTAMP, "deniedAt" TIMESTAMP, "denialReason" text, "amount" numeric(10,2), "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_96c91970c0dcb2f69fdccd0a698" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE INDEX IF NOT EXISTS "IDX_f019aca30a997d89b0b76031c7" ON "claims" ("enrollmentId") `);
        await queryRunner.query(`CREATE INDEX IF NOT EXISTS "IDX_3feefa4a032507a422e140b606" ON "claims" ("patientId", "status") `);
        await this.safeCreateType(queryRunner, `CREATE TYPE "public"."ai_drafts_drafttype_enum" AS ENUM('monthly_note', 'call_script', 'risk_summary', 'claim_justification', 'triage_summary')`);
        await this.safeCreateType(queryRunner, `CREATE TYPE "public"."ai_drafts_status_enum" AS ENUM('draft', 'reviewed', 'accepted', 'discarded')`);
        await queryRunner.query(`CREATE TABLE IF NOT EXISTS "ai_drafts" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "patientId" uuid NOT NULL, "enrollmentId" uuid, "programType" character varying, "draftType" "public"."ai_drafts_drafttype_enum" NOT NULL, "promptVersion" character varying NOT NULL, "modelVersion" character varying NOT NULL, "inputRefs" jsonb, "outputText" text NOT NULL, "status" "public"."ai_drafts_status_enum" NOT NULL DEFAULT 'draft', "reviewedByUserId" uuid, "reviewedAt" TIMESTAMP, "reviewNotes" text, "usedInNoteId" uuid, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_a83622f0d6fd52872e9585de51f" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE INDEX IF NOT EXISTS "IDX_ac9f1bb6114dcb51d6d1f05620" ON "ai_drafts" ("status") `);
        await queryRunner.query(`CREATE INDEX IF NOT EXISTS "IDX_6046218387bd1cabd8c00e7f3d" ON "ai_drafts" ("patientId", "draftType") `);
        await this.safeAddColumn(queryRunner, 'vital_readings', 'timestampDevice', 'TIMESTAMP');
        await this.safeAddColumn(queryRunner, 'vital_readings', 'timestampReceived', 'TIMESTAMP');
        await this.safeAddColumn(queryRunner, 'vital_readings', 'isValid', 'boolean NOT NULL DEFAULT true');
        await this.safeAddColumn(queryRunner, 'vital_readings', 'isOutlier', 'boolean NOT NULL DEFAULT false');
        await this.safeAddColumn(queryRunner, 'vital_readings', 'qualityScore', 'integer');
        await this.safeAddColumn(queryRunner, 'users', 'licenseType', 'character varying');
        await this.safeAddColumn(queryRunner, 'users', 'licenseNumber', 'character varying');
        await this.safeAddColumn(queryRunner, 'users', 'credentialingStatus', 'character varying');
        await this.safeAddColumn(queryRunner, 'communication_logs', 'interactiveFlag', 'boolean NOT NULL DEFAULT false');
        await this.safeAddColumn(queryRunner, 'communication_logs', 'timeEntryId', 'uuid');
        await this.safeAddColumn(queryRunner, 'audit_logs', 'actorType', 'character varying');
        await this.safeAddColumn(queryRunner, 'audit_logs', 'beforeHash', 'character varying');
        await this.safeAddColumn(queryRunner, 'audit_logs', 'afterHash', 'character varying');
        await this.safeAddColumn(queryRunner, 'audit_logs', 'prevAuditHash', 'character varying');
        await this.safeAddColumn(queryRunner, 'audit_logs', 'auditHash', 'character varying');
        // Upgrade users_role_enum only if 'nurse' value doesn't already exist
        const hasNurseRole = await queryRunner.query(`SELECT 1 FROM pg_enum WHERE enumlabel = 'nurse' AND enumtypid = (SELECT oid FROM pg_type WHERE typname = 'users_role_enum') LIMIT 1`);
        if (hasNurseRole.length === 0) {
        ...
        // Upgrade invite_codes_allowedrole_enum only if 'nurse' value doesn't already exist
        const hasNurseInvite = await queryRunner.query(`SELECT 1 FROM pg_enum WHERE enumlabel = 'nurse' AND enumtypid = (SELECT oid FROM pg_type WHERE typname = 'invite_codes_allowedrole_enum') LIMIT 1`);
        if (hasNurseInvite.length === 0) {
