...
export class InitialSchema1771935772168 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await this.safeCreateType(
      queryRunner,
      `CREATE TYPE "public"."time_entries_category_enum" AS ENUM('monitoring', 'outreach', 'documentation', 'care_coordination', 'education')`,
    );
    await this.safeCreateType(
      queryRunner,
      `CREATE TYPE "public"."time_entries_source_enum" AS ENUM('manual', 'assisted', 'system')`,
    );
    await this.safeCreateType(
      queryRunner,
      `CREATE TYPE "public"."time_entries_status_enum" AS ENUM('draft', 'confirmed', 'billed')`,
    );
    await queryRunner.query(
      `CREATE TABLE IF NOT EXISTS "time_entries" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "patientId" uuid NOT NULL, "userId" uuid NOT NULL, "enrollmentId" uuid, "programType" character varying, "startAt" TIMESTAMP NOT NULL, "endAt" TIMESTAMP, "minutes" integer NOT NULL DEFAULT '0', "category" "public"."time_entries_category_enum" NOT NULL, "source" "public"."time_entries_source_enum" NOT NULL DEFAULT 'manual', "taskId" uuid, "communicationLogId" uuid, "noteId" uuid, "evidenceRefs" text, "billable" boolean NOT NULL DEFAULT true, "cptCode" character varying, "status" "public"."time_entries_status_enum" NOT NULL DEFAULT 'draft', "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_b8bc5f10269ba2fe88708904aa0" PRIMARY KEY ("id"))`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "IDX_7b179edfc496340bb52aa6f4fa" ON "time_entries" ("userId", "createdAt") `,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "IDX_fe5c4ded8ae61869d816a15465" ON "time_entries" ("patientId", "createdAt") `,
    );
    await this.safeCreateType(
      queryRunner,
      `CREATE TYPE "public"."tasks_type_enum" AS ENUM('review', 'outreach', 'escalation', 'setup', 'billing_review')`,
    );
    await this.safeCreateType(
      queryRunner,
    ...
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "IDX_1aa2ea5bb323043bab51bbdfbc" ON "tasks" ("assignedToUserId", "status") `,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "IDX_bdb2dc49204bc5229c8cab5e17" ON "tasks" ("patientId", "status") `,
    );
    await queryRunner.query(
      `CREATE TABLE IF NOT EXISTS "threshold_policies" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "clinicId" uuid NOT NULL, "programType" character varying NOT NULL, "version" integer NOT NULL DEFAULT '1', "isActive" boolean NOT NULL DEFAULT true, "rules" jsonb NOT NULL, "effectiveFrom" TIMESTAMP NOT NULL, "effectiveTo" TIMESTAMP, "createdBy" uuid NOT NULL, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_3680e23497ba63477f05b2dbe6b" PRIMARY KEY ("id"))`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "IDX_c6084d968cb7fc1b4c3e7aaf86" ON "threshold_policies" ("isActive") `,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "IDX_8541053809c82c7fff17751f38" ON "threshold_policies" ("clinicId", "programType") `,
    );
    await this.safeCreateType(
      queryRunner,
      `CREATE TYPE "public"."enrollments_programtype_enum" AS ENUM('BP', 'GLUCOSE')`,
    );
    await this.safeCreateType(
      queryRunner,
      `CREATE TYPE "public"."enrollments_status_enum" AS ENUM('active', 'paused', 'completed', 'cancelled')`,
    );
    await queryRunner.query(
    ...
    const hasNurseRole = await queryRunner.query(
      `SELECT 1 FROM pg_enum WHERE enumlabel = 'nurse' AND enumtypid = (SELECT oid FROM pg_type WHERE typname = 'users_role_enum') LIMIT 1`,
    );
    if (hasNurseRole.length === 0) {
      ...
      await queryRunner.query(
        `CREATE TYPE "public"."users_role_enum" AS ENUM('patient', 'provider', 'nurse', 'clinical_staff', 'billing_staff', 'admin', 'superadmin', 'compliance_auditor')`,
      );
    ...
    const hasNurseInvite = await queryRunner.query(
      `SELECT 1 FROM pg_enum WHERE enumlabel = 'nurse' AND enumtypid = (SELECT oid FROM pg_type WHERE typname = 'invite_codes_allowedrole_enum') LIMIT 1`,
    );
    if (hasNurseInvite.length === 0) {
      await queryRunner.query(
        `ALTER TYPE "public"."invite_codes_allowedrole_enum" RENAME TO "invite_codes_allowedrole_enum_old"`,
      );
      await queryRunner.query(
        `CREATE TYPE "public"."invite_codes_allowedrole_enum" AS ENUM('patient', 'provider', 'nurse', 'clinical_staff', 'billing_staff', 'admin', 'superadmin', 'compliance_auditor')`,
      );
      await queryRunner.query(`ALTER TABLE "invite_codes" ALTER COLUMN "allowedRole" DROP DEFAULT`);
      await queryRunner.query(
        `ALTER TABLE "invite_codes" ALTER COLUMN "allowedRole" TYPE "public"."invite_codes_allowedrole_enum" USING "allowedRole"::"text"::"public"."invite_codes_allowedrole_enum"`,
      );
      await queryRunner.query(
        `ALTER TABLE "invite_codes" ALTER COLUMN "allowedRole" SET DEFAULT 'provider'`,
      );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "public"."invite_codes_allowedrole_enum_old" AS ENUM('patient', 'provider', 'admin', 'superadmin')`,
    );
    await queryRunner.query(`ALTER TABLE "invite_codes" ALTER COLUMN "allowedRole" DROP DEFAULT`);
    await queryRunner.query(
      `ALTER TABLE "invite_codes" ALTER COLUMN "allowedRole" TYPE "public"."invite_codes_allowedrole_enum_old" USING "allowedRole"::"text"::"public"."invite_codes_allowedrole_enum_old"`,
    );
    await queryRunner.query(
      `ALTER TABLE "invite_codes" ALTER COLUMN "allowedRole" SET DEFAULT 'provider'`,
    );
    ...
    await queryRunner.query(
      `ALTER TYPE "public"."invite_codes_allowedrole_enum_old" RENAME TO "invite_codes_allowedrole_enum"`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."users_role_enum_old" AS ENUM('patient', 'provider', 'admin', 'superadmin')`,
    );
    await queryRunner.query(`ALTER TABLE "users" ALTER COLUMN "role" DROP DEFAULT`);
    await queryRunner.query(
