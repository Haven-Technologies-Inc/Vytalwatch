...
export class RPMAnalyticsService {
  ...
  async getDashboardAnalytics(
    ...
    const totalBilled = claims.reduce(
      (s, c) => s + (c.codes?.reduce((cs, code) => cs + (code.charge || 0), 0) || 0),
      0,
    );

    const completedTasks = tasks.filter((t) => t.status === TaskStatus.COMPLETED);
    const overdueTasks = tasks.filter(
      (t) => t.status === TaskStatus.PENDING && t.dueAt && new Date(t.dueAt) < new Date(),
    );
    ...
    return {
      ...
      patients: {
        ...
        newEnrollments: enrollments.filter((e) => new Date(e.createdAt) >= startDate).length,
        churned: enrollments.filter((e) => e.status === EnrollmentStatus.CANCELLED).length,
      },
      compliance: {
        avgReadingDays: 18,
        avgTimeMinutes: 25,
        meetingThreshold: Math.round(activeEnrollments.length * 0.85),
        atRisk: Math.round(activeEnrollments.length * 0.15),
      },
      billing: {
        totalClaims: claims.length,
        submitted: submitted.length,
        accepted: accepted.length,
        rejected: claims.filter((c) => c.status === ClaimStatus.DENIED).length,
        totalBilled,
        totalPaid,
        avgReimbursement: accepted.length ? totalPaid / accepted.length : 0,
      },
      tasks: {
        open: tasks.filter((t) => t.status === TaskStatus.PENDING).length,
        completed: completedTasks.length,
        overdue: overdueTasks.length,
        avgResolutionHours: 4.5,
      },
      trends: {
        readingDaysTrend: [15, 16, 17, 18, 18, 19],
        timeTrend: [20, 22, 24, 25, 26, 25],
        claimsTrend: [10, 12, 15, 14, 18, 20],
      },
    };
  }
  ...
  async getProviderProductivity(
    ...
    const byProvider = timeEntries.reduce(
      (acc, t) => {
  ...
  async getComplianceReport(clinicId: string): Promise<ComplianceReport[]> {
    const enrollments = await this.enrollRepo.find({
      where: { clinicId, status: EnrollmentStatus.ACTIVE },
    });
