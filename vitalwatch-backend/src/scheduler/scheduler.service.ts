...
export class SchedulerService implements OnModuleDestroy {
  ...
  async handleBillingCycle(): Promise<void> {
    ...
    try {
          ...
          if (device.totalReadings >= 16) {
            ...
            await this.billingService.createBillingRecord({
              ...
              supportingData: {
                ...
                interactionNotes: [
                  `Device: ${device.id}`,
                  `Period: ${firstOfMonth.toISOString()} - ${lastOfMonth.toISOString()}`,
                ],
              },
            });
      ...
      this.logger.error(`[${jobName}] Fatal error: ${error.message}`, error.stack);
    }
  }
  ...
  async handleReportGeneration(): Promise<void> {
    ...
    try {
      ...
      this.logger.log(`[${jobName}] Found ${pendingReports.length} pending reports to process`);
  ...
  async handleDeviceHealthCheck(): Promise<void> {
    ...
    try {
      ...
      const staleDevices = await this.deviceRepository
        ...
        .andWhere('(device.lastSyncAt IS NULL OR device.lastSyncAt < :cutoff)', {
      ...
      for (const device of staleDevices) {
        try {
          ...
          const lastSync = device.lastSyncAt ? device.lastSyncAt.toISOString() : 'never';
      ...
      const elapsed = Date.now() - startTime;
      this.logger.log(`[${jobName}] Completed in ${elapsed}ms. Alerts created: ${alertsCreated}`);
    } catch (error) {
      this.logger.error(`[${jobName}] Fatal error: ${error.message}`, error.stack);
    }
  }
